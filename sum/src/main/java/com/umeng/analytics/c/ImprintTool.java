...
import com.yxd.sum.bean.ImprintValue;
...
public class ImprintTool {
    ...
    public String checkSum(Imprint imprint) {
        ...
        return EncodeTool.getMD5_2(sb.toString()).toLowerCase(Locale.US);
    }
        ...
        if(file.exists()) {
            ...
            if(data != null) {
                try {
                    ...
                    (new NBeanUnpacker()).unpack(imprint, data);
    ...
    public void writeToCache() {
        if(this.imprint != null) {
            try {
                byte[] data = (new NBeanPacker()).pack2Bytes(this.imprint);
                EncodeTool.writeToFile(new File(this.context.getFilesDir(), ".imprint"), data);
            } catch (Exception e) {
                e.printStackTrace();
            }
