    ...
    private IdTracking readFromFile() {
        if(!this.file.exists()) {
            ...
            try {
                ...
                byte[] data = EncodeTool.readData(fis);
                IdTracking idTracking = new IdTracking();
                (new NBeanUnpacker()).unpack(idTracking, data);
                return idTracking;
            } catch (Exception e) {
                e.printStackTrace();
            } finally {
    ...
    private void writeToFile(IdTracking idTracking) {
        if(idTracking != null) {
            try {
                byte[] data;
                synchronized(this) {
                    data = (new NBeanPacker()).pack2Bytes(idTracking);
                }

                if(data != null) {
                    EncodeTool.writeToFile(this.file, data);
                }
            } catch (Exception e) {
                e.printStackTrace();
            }
