...
import com.yxd.sum.bean.Imprint;
...
public final class LogReporter implements Reporter, OptionSetter {
    ...
    private ReportPolicy reportPolicy = null;
    private ImprintTool.Option option = null;
    private long ts = 0L;
    private static Context context;
    public String track_list = null;
    ...
    public LogReporter(Context context) {
        ...
        this.optionSetter_a = OptionSetter_a.a(LogReporter.context);
        this.optionSetter_b = OptionSetter_b.a(LogReporter.context);
        this.optionSetter_c = OptionSetter_c.a(LogReporter.context, this.requestTracker);
        SharedPreferences sp = SPTool.getSp(LogReporter.context);
        this.lastTime = sp.getLong("thtstart", 0L);
        this.count1 = sp.getInt("gkvc", 0);
        this.count2 = sp.getInt("ekvc", 0);
        this.track_list = ImprintTool.getInstance(LogReporter.context).getOption().getTrack_list(null);
    }
    ...
    private void envelope(JSONObject jsonObject) {
        try {
            ...
            try {
                ...
                byte[] var4 = (new NBeanPacker()).pack2Bytes(idTracking);
    ...
    protected JSONObject packEntity(int... policys) {
        ...
        try {
            if(TextUtils.isEmpty(AnalyticsConfig.getAppkey(context))) {
                ...
                if(policys != null && policys.length == 2) {
                    active_userJson = new JSONObject();
                    JSONObject latent = new JSONObject();
                    latent.put("interval", policys[0] / 1000);
                    latent.put("latency", policys[1]);
                ...
                String var28 = EncodeTool.getMD5_2(AnalyticsConfig.getSecretKey(context));
                ...
                try {
                    ...
                    if(imprint != null) {
                        byte[] imprintData = (new NBeanPacker()).pack2Bytes(imprint);
    ...
    private boolean canReport(boolean var1) {
        if(!SystemUtil.isConnectState(context)) {
            ...
            return this.requestTracker.hasNotRequest()?true:this.reportPolicy.getUMPolicy(var1).canReport(var1);
        }
    }
    ...
    private void startReport() {
        try {
            if(this.cacheTool.hasCache()) {
                DataReportor logReportor = new DataReportor(context, this.requestTracker);
                ...
                JSONObject jsonObject = this.packEntity(new int[0]);
    ...
    public void setOption(ImprintTool.Option option) {
        ...
        this.reportPolicy.setOption(option);
    ...
    public void saveData(Context context) {
        try {
            if(LogReporter.context == null) {
                LogReporter.context = context;
            }

            if(this.jsonArray.length() > 0) {
                DBDataTool.getInstance(LogReporter.context).insertToEt(this.jsonArray);
                this.jsonArray = new JSONArray();
            }
    ...
    public class ReportPolicy {
        ...
        public ReportPolicy() {
            int[] var2 = LogReporter.this.option.getReportPolicy(-1, -1);
            this.policy = var2[0];
        ...
        protected void a(boolean var1) {
            boolean var2;
            if(LogReporter.this.optionSetter_b.isDiscardOnFail()) {
                var2 = this.umPolicy instanceof J.UMPolicy_b && this.umPolicy.canDiscard();
                this.umPolicy = var2?this.umPolicy :new J.UMPolicy_b(LogReporter.this.requestTracker, LogReporter.this.optionSetter_b);
            } else {
                var2 = this.umPolicy instanceof J.UMPolicy_c && this.umPolicy.canDiscard();
                if(!var2) {
                    if(var1 && LogReporter.this.optionSetter_c.isNeedReport()) {
                        this.umPolicy = new J.UMPolicy_c((int)LogReporter.this.optionSetter_c.b());
                        LogReporter.this.startDelayReport((int)LogReporter.this.optionSetter_c.b());
                    } else if(ULog.isDebugMode && LogReporter.this.option.isIntegrated_test()) {
                        this.umPolicy = new J.UMPolicy_a(LogReporter.this.requestTracker);
                    } else {
                        int policy;
                        if(LogReporter.this.optionSetter_a.a() && "RPT".equals(LogReporter.this.optionSetter_a.f())) {
                            policy = 0;
                            if(LogReporter.this.optionSetter_a.getPolicy() == 6) {
                                if(LogReporter.this.option.hasTest_report_interval()) {
                                    policy = LogReporter.this.option.getTest_report_interval(90000);
                            ...
                            this.umPolicy = this.getUMPolicy(LogReporter.this.optionSetter_a.getPolicy(), policy);
                        } else {
        ...
        private J.UMPolicy getUMPolicy(int policy, int interval) {
            ...
            switch(policy) {
                ...
                default:
                    ...
                    break;
                case 4:
                    policy1 = this.umPolicy instanceof J.UMPolicy_f ?this.umPolicy :new J.UMPolicy_f(LogReporter.this.requestTracker);
                    break;
                case 5:
                    policy1 = this.umPolicy instanceof J.UMPolicy_i ?this.umPolicy :new J.UMPolicy_i(LogReporter.context);
                    break;
                case 6:
                    if(this.umPolicy instanceof J.UMPolicy_e) {
                        ...
                        policy1 = new J.UMPolicy_e(LogReporter.this.requestTracker, (long)interval);
                    }
                    break;
                case 8:
                    policy1 = this.umPolicy instanceof UMPolicy_j ?this.umPolicy :new UMPolicy_j(LogReporter.this.requestTracker);
            }
